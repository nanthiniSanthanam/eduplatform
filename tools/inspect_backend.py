...
Notes:
    ...
    - Set ANALYZER_MODE=1 environment variable to prevent Django side effects
...
class BackendAnalyzer:
    ...
    def __init__(self, backend_path, verbose=False, exclude_apps=None, max_issues_to_show=100,
        ...
        if use_django_reflection:
            self.django_ready, self.django_error = setup_django_environment(self.backend_path)
            if self.django_ready:
                ...
                try:
                    ...
                    self.log(f"Error importing Django modules: {str(e)}", "error")
            else:
                self.log(f"Django environment not initialized: {self.django_error}", "warning")
        ...
        if config_file and os.path.exists(config_file):
            try:
                ...
                self.log(f"Error loading config file {config_file}: {str(e)}", "error")
            ...
            for module_name, module in sys.modules.items():
                ...
                for name, obj in inspect.getmembers(module):
                    if isinstance(obj, BaseRouter):
                        ...
                        self.log(f"Found router: {name} in {module_name}", "info")
        ...
        if self.django_ready:
            try:
                ...
                for app_config in self.django_apps.get_app_configs():
                    app_name = app_config.name.split('.')[-1]  # Get the last part of the dotted path
                    self.apps.add(app_name)
                    self.log(f"Found Django app via registry: {app_name}", "success")
                
                # After finding all apps
                self.apps = {app for app in self.apps if app not in self.exclude_apps}
                self.log(f"Total Django apps found via registry: {len(self.apps)}")
                return self.apps
            except Exception as e:
            ...
            try:
                ...
                for model in self.django_apps.get_models():
                    try:
                        ...
                        model_info = self._extract_model_info_from_django(model)
                        self.models[f"{app_name}.{model_name}"] = model_info
                        self.log(f"  Found model via registry: {model_name} with {len(model_info.fields)} fields", "success")
                    except Exception as e:
                        self.log(f"Error analyzing model {model.__name__}: {str(e)}", "error")
                
                self.log(f"Total models found via registry: {len(self.models)}")
                return self.models
            except Exception as e:
                self.log(f"Error using Django model registry: {str(e)}", "error")
    ...
    def _extract_model_info_from_django(self, model):
        ...
        for field in model._meta.get_fields():
            try:
                    ...
                    if hasattr(field, option):
                        ...
                        if value not in [None, False, ''] and not callable(value):
                            ...
                            if option == 'default' and callable(value):
                                ...
                                try:
                                    json.dumps({option: value})  # Test JSON serialization
                ...
                self.log(f"Error extracting field {field.name} from {model_name}: {str(e)}", "error")
    ...
    def analyze_urls(self):
        ...
        if self.django_ready:
            try:
                ...
                url_patterns = self._process_url_patterns(self.django_urls.url_patterns, prefix='')
                ...
                self.log(f"Total URL patterns found via resolver: {total_urls}")
                return self.urls
            except Exception as e:
    ...
    def _process_url_patterns(self, url_patterns, prefix=''):
        ...
        for pattern in url_patterns:
            if isinstance(pattern, URLPattern):
                ...
                pattern_str = prefix + self._format_pattern_for_url(pattern.pattern)
                view_str = self._get_view_string(pattern.callback)
                name = pattern.name
                
                url_info = URLInfo(pattern_str, view_str, name)
                results.append(url_info)
                self.log(f"  Found URL pattern: {pattern_str} → {view_str}", "success")
                
            elif isinstance(pattern, URLResolver):
                # Process included URL patterns recursively
                new_prefix = prefix + self._format_pattern_for_url(pattern.pattern)
                nested_patterns = self._process_url_patterns(pattern.url_patterns, new_prefix)
        ...
        for router_info in self.routers:
            ...
            try:
                ...
                for prefix, viewset, basename in router.registry:
                    self.log(f"  Found router registration: {prefix} → {viewset.__name__}", "success")
                    ...
                    for action, method in viewset_actions.items():
                        ...
                        if action == 'list':
                            ...
                            detail = getattr(getattr(viewset, action, None), 'detail', False)
                ...
                self.log(f"Error processing router {router_name}: {str(e)}", "error")
    ...
    def detect_compatibility_issues(self):
        ...
        for base_path, endpoints in endpoint_groups.items():
            ...
            auth_required = [e for e in endpoints if e.permissions and any('IsAuthenticated' in p for p in e.permissions)]
            not_auth_required = [e for e in endpoints if not e.permissions or not any('IsAuthenticated' in p for p in e.permissions)]
    ...
    def generate_report(self, analysis_data, output_format='markdown'):
        ...
        if output_format == 'json':
            ...
            security_matrix = self.generate_security_matrix()
            if security_matrix:
                report_lines.extend(self._format_security_matrix(security_matrix))
            else:
            ...
            if hasattr(self, 'django_version'):
                report_lines.append(f"- **Django Version**: {self.django_version}")
...
"""
    ...
    def generate_openapi_schema(self):
        ...
        if not self.django_ready:
            self.log("Django environment not initialized, cannot generate OpenAPI schema", "error")
            return None
        ...
        if not schema and self.api_endpoints:
            self.log("Generating basic OpenAPI schema from collected endpoints", "info")
    ...
    def _generate_basic_openapi_schema(self):
        ...
        for endpoint in self.api_endpoints:
            ...
            schema["paths"][path][method] = self._generate_operation_object(endpoint)
...
"""
        ...
        for serializer_key, serializer_info in self.serializers.items():
            ...
            if model_name:
                ...
                if serializer_name.endswith('Serializer'):
                    interface_name = serializer_name[:-10]  # Remove 'Serializer' suffix
                else:
                ...
                if serializer_info.meta.get('fields') != '__all__' or serializer_info.meta.get('exclude'):
                    serializer_interfaces.append(self._generate_serializer_interface(serializer_info, interface_name))
    ...
    def generate_sample_config(self, output_file="backend_analyzer_config.json"):
        ...
        try:
            ...
            self.log(f"Sample configuration file generated: {output_file}", "success")
            return True
        except Exception as e:
            self.log(f"Error generating sample configuration file: {str(e)}", "error")
            return False
    ...
    def _generate_model_interface(self, model_info):
        ...
        for field in model_info.fields:
            ...
            # Determine if field is required
            is_optional = field.options.get('blank', False) or field.options.get('null', False)
            ...
            fields.append(f"{readonly}{field_name}{optional_marker}: {ts_type}")
...
"""
    ...
    def generate_security_matrix(self):
        ...
        if not auth_mechanisms and matrix['auth_required']:
            self.log(f"Warning: {len(matrix['auth_required'])} endpoints require authentication, but no authentication mechanism is configured", "warning")
            for endpoint in matrix['auth_required']:
                self.log(f"  {endpoint['method']} {endpoint['path']} requires authentication", "warning")
        ...
        for base_path, endpoints in endpoint_groups.items():
            ...
            not_auth_required = [e for e in endpoints if not e['requires_auth']]
    ...
    def _format_security_matrix(self, matrix):
        ...
        lines.append(f"- Unique permission classes: {len(matrix['permissions'])}")
        lines.append("")
        ...
        lines.append("")
        lines.append("| Permission Class | Endpoints Using | Authentication Required |")
        lines.append("| --- | --- | --- |")
        
        for perm in matrix['permissions']:
            # Count endpoints using this permission
            count = sum(1 for e in matrix['endpoints'] if perm in e['permissions'])
            # Check if this permission requires authentication
            auth_required = "Yes" if any(keyword in perm for keyword in ['IsAuthenticated', 'TokenHasScope', 'IsAdminUser']) else "No"
        ...
        if matrix['auth_required']:
            ...
            for endpoint in matrix['auth_required']:
                perms = ', '.join(endpoint['permissions']) if endpoint['permissions'] else 'Default'
                lines.append(f"| {endpoint['method']} | {endpoint['path']} | {perms} |")
        ...
        if matrix['public']:
            ...
            for endpoint in matrix['public']:
                perms = ', '.join(endpoint['permissions']) if endpoint['permissions'] else 'None'
                lines.append(f"| {endpoint['method']} | {endpoint['path']} | {perms} |")
        ...
        if args.output_typescript:
            ...
            if typescript:
                with open(args.typescript_file, 'w') as f:
                    f.write(typescript)
                print(f"TypeScript interfaces generated: {args.typescript_file}")
            else:
