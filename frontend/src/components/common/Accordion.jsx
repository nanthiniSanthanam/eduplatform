...
const AccordionItem = ({
  ...
  return (
    <div className={`rounded-lg overflow-hidden mb-3 ${borderClasses} ${className}`}>
      <button
        className="w-full flex items-center justify-between p-4 text-left bg-white hover:bg-gray-50 transition-colors"
        onClick={() => toggleItem(index)}
      >
        <h3 className="font-medium text-gray-900">{title}</h3>
        <i className={`fas ${isOpen ? 'fa-chevron-up' : 'fa-chevron-down'} text-gray-500 transition-transform`}></i>
      </button>
      
      <div className={`bg-gray-50 transition-all overflow-hidden ${isOpen ? 'max-h-96 p-4' : 'max-h-0'}`}>
        {children}
      </div>
    </div>
  );
