...
const BookmarksList = ({ courseSlug, onNavigate, className = '', emptyMessage = 'No bookmarks yet' }) => {
  ...
  useEffect(() => {
    ...
    const loadBookmarks = () => {
      try {
        ...
        // Validate bookmarks
        const validBookmarks = Array.isArray(savedBookmarks) ? savedBookmarks.filter(bookmark => 
          bookmark && 
          bookmark.lessonId && 
          bookmark.moduleId &&
          bookmark.timestamp
  ...
  const navigateToBookmark = (bookmark) => {
    ...
    navigate(`/courses/${courseSlug}/content/${bookmark.moduleId}/${bookmark.lessonId}`);
  ...
  if (bookmarks.length === 0) {
    return (
      <div className={`text-center py-6 text-gray-500 ${className}`}>
        <p>{emptyMessage}</p>
        <p className="text-sm mt-1">Bookmark lessons to quickly access them later</p>
      </div>
    );
  }
      ...
      {bookmarks.map((bookmark, index) => (
        ...
        >
          <div className="flex-1">
            <h4 className="text-sm font-medium text-gray-900">{bookmark.title || `Lesson ${bookmark.lessonId}`}</h4>
            <p className="mt-1 text-xs text-gray-500">Bookmarked {formatTimestamp(bookmark.timestamp)}</p>
          </div>
          <button 
