...
export default function CourseContentRouteChecker() {
  ...
  const { isAuthenticated, userRole, userAccessLevel, isLoading: authLoading } = useAuth();
  
  const [loading, setLoading] = useState(true);
  const [timedOut, setTimedOut] = useState(false);
  
  // Determine if user is instructor (simple check)
  const isInstructor = userRole === 'instructor' || userRole === 'admin' || userRole === 'administrator';
  
  useEffect(() => {
    console.log('CourseContentRouteChecker mounting with params:', { courseSlug, moduleId, lessonId });
    console.log('Auth state:', { isAuthenticated, userRole, userAccessLevel, authLoading });
    
    // Set emergency timeout
    const timeoutId = setTimeout(() => {
      console.warn('EMERGENCY TIMEOUT: Bypassing permission checks after 2 seconds');
      setLoading(false);
      setTimedOut(true);
    }, EMERGENCY_TIMEOUT);
  ...
  }, [courseSlug, moduleId, lessonId, isInstructor, authLoading, timedOut, isAuthenticated, userRole, userAccessLevel]);
  ...
  if (userAccessLevel === LEVELS.BASIC && !isAuthenticated) {
    // If the user is not logged in and tries to access intermediate content, redirect to login
    return <Navigate to={`/login?redirect=${encodeURIComponent(location.pathname)}`} replace />;
  }
  
  // For advanced content
  if (userAccessLevel !== LEVELS.ADVANCED) {
    // If the content requires advanced access and user doesn't have it,
