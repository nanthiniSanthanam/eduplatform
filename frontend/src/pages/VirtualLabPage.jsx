    ...
    const fetchLabData = async () => {
      try {
        ...
        const mockLab = {
          ...
          description: "In this lab, you'll build a lightweight state management system for React applications similar to Redux but with a simpler API. You'll implement actions, reducers, and a store along with React hooks to connect components.",
          courseId: 1,
...
export default function App() {
  return (
    <StoreProvider store={store}>
      <div className="app">
          ...
          tasks: [
            {
              ...
              description: "Create a store factory function that returns an object with getState, dispatch, and subscribe methods.",
              ...
              testCriteria: "The createStore function should properly manage state and allow state modifications via dispatched actions."
            },
            {
              id: 2,
              title: "Implement the StoreProvider component",
              description: "Create a React context provider that makes the store accessible to all child components.",
              hint: "Use React.createContext and context.Provider to share the store instance with the component tree.",
              testCriteria: "The StoreProvider should properly pass the store instance through context."
            },
            {
              id: 3,
              title: "Implement the useStore, useDispatch, and useSelector hooks",
              description: "Create custom hooks that allow components to interact with the store.",
              ...
              testCriteria: "Components should be able to read from the store and dispatch actions to update the state."
            }
          ]
        };
        ...
        // Set initial data
        setLab(mockLab);
        setActiveFile(mockLab.files.find(file => file.isMainFile) || mockLab.files[0]);
        setCode(mockLab.files.find(file => file.isMainFile)?.content || mockLab.files[0].content);
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      if (lab.language === 'javascript') {
        if (code.includes('getState') && code.includes('dispatch') && code.includes('subscribe')) {
          ...
          simulatedOutput = "⚠️ Your implementation is missing some required functionality.\n\n";
        ...
        simulatedOutput = "Execution for this language is not available in the demo.";
      }
  ...
  const handleCheckTask = async () => {
    ...
    try {
      ...
      const taskPassed = currentTask === 0 ? 
        code.includes('getState') && code.includes('dispatch') && code.includes('subscribe') :
        currentTask === 1 ? 
          code.includes('StoreContext.Provider') && code.includes('value={store}') :
          code.includes('useStore') && code.includes('useDispatch') && code.includes('useSelector');
  ...
  const handleSubmitLab = async () => {
    ...
    try {
      ...
      setFeedback({
        type: 'error',
        message: 'An error occurred while submitting your lab. Please try again.'
      });
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className="min-h-screen bg-gray-900 text-white py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-gray-300">Loading virtual lab environment...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-gray-800 border-b border-gray-700 px-4 py-2 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <span className="text-xs px-2 py-1 rounded-full bg-blue-900 text-blue-200">
            {lab.language === 'javascript' ? 'JavaScript' : 
             lab.language === 'python' ? 'Python' : lab.language}
          </span>
        </div>
      ...
      <div className="flex-grow flex overflow-hidden">
        ...
        <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col overflow-hidden">
          ...
          <div className="flex-grow overflow-y-auto p-4">
            {showInstructions ? (
              <div>
                <h3 className="font-medium text-lg">{lab.tasks[currentTask].title}</h3>
                <p className="mt-2 text-gray-300 text-sm">{lab.tasks[currentTask].description}</p>
                
                {showHint && (
                  <div className="mt-4 p-3 bg-gray-700 rounded-md text-sm">
                    <h4 className="font-medium text-yellow-400 mb-1">Hint:</h4>
                    <p className="text-gray-300">{lab.tasks[currentTask].hint}</p>
                  </div>
                )}
                ...
                <h4 className="font-medium mt-6 mb-2 text-gray-200">Test Criteria:</h4>
                <p className="text-gray-300 text-sm">{lab.tasks[currentTask].testCriteria}</p>
                
                <h4 className="font-medium mt-6 mb-2 text-gray-200">Lab Objectives:</h4>
                  ...
                  {completedTasks.length === lab.tasks.length && (
                    <button
                      ...
                      className={`w-full mt-3 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${
                        (isSubmitting || isLabCompleted) ? 'opacity-70 cursor-not-allowed' : ''
                      }`}
                    >
                      {isSubmitting ? 'Submitting...' : isLabCompleted ? 'Lab Completed' : 'Submit Lab'}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="text-center py-16">
                ...
                <p className="mt-2 text-sm text-gray-300">Click "Show Instructions" to see task details</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-gray-800 border-b border-gray-700 px-1">
            <div className="flex overflow-x-auto">
              {lab.files.map(file => (
                ...
                >
                  {file.name}
                  {file.readOnly && <span className="ml-1 text-xs text-gray-500">(read-only)</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-gray-800 border-t border-gray-700">
            ...
            <div className="flex justify-between items-center px-4 py-2 border-b border-gray-700">
              <div className="flex space-x-2">
                ...
                <button
                  onClick={() => {
                    if (activeFile && !activeFile.readOnly && confirm('Reset this file to its initial state?')) {
                      const originalFile = lab.files.find(f => f.id === activeFile.id);
                      if (originalFile) {
