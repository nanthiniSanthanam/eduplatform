  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      try {
        ...
        console.log('Received code:', code ? `${code.substring(0, 8)}...` : 'none');
        ...
        if (storedCode === code && expiry && Date.now() < parseInt(expiry, 10)) {
          console.log('OAuth code already processed, skipping duplicated processing');
        ...
        sessionStorage.setItem('oauth_code_expiry', (Date.now() + 5*60*1000).toString()); // 5 minutes
        
        setStatus(`Completing ${provider} authentication...`);
        console.log(`Processing ${provider} callback with code: ${code.substring(0, 10)}...`);
        ...
        if (!state && localStorage.getItem('oauth_state')) {
          ...
          const bypassState = process.env.NODE_ENV === 'development' ? 'bypass-for-dev' : state;
          
          // Process social auth with state parameter for CSRF protection
          const userData = await auth.handleSocialAuthCallback(provider, code, bypassState);
          ...
          const userData = await auth.handleSocialAuthCallback(provider, code, state);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
      {error ? (
        <div className="text-center p-8 max-w-md">
          <div className="mb-4 text-red-600 text-lg font-medium">Authentication Failed</div>
        ...
        <div className="text-center p-8">
          ...
          <div className="mt-4 text-xl font-semibold text-gray-700">{status}</div>
          <div className="mt-2 text-gray-500">Please wait while we complete your sign in...</div>
        </div>
      )}
    </div>
  );
