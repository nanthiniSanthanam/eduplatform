 ...
 * 
 * Updated: 2025-05-20 17:38:09 - Updated social login implementation
 */
...
const STRINGS = {
  ...
  FORGOT_PASSWORD_PROMPT: 'Having trouble signing in? You can reset your password.',
  SHOW_PASSWORD: 'Show password',
  HIDE_PASSWORD: 'Hide password',
  SOCIAL_LOGIN_ERROR: 'Social login failed. Please try again or use email login.',
};
...
const LoginPage = () => {
  ...
  useEffect(() => {
    if (auth.isAuthenticated && auth.currentUser) {
      const dashboardRoute = getDashboardRoute(auth.currentUser.role || 'student');
...
const handleSocialLogin = async (provider) => {
  try {
    ...
    if (provider === 'google') {
      const clientId = '99067790447-go0pcefo3nt1b9d0udei65u0or6nb0a0.apps.googleusercontent.com'; // Replace with your real client ID
      const redirectUri = encodeURIComponent(`${window.location.origin}/auth/social/google/callback`);
