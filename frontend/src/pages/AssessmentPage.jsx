...
const AssessmentPage = () => {
    ...
    const fetchAssessment = async () => {
      try {
        ...
        const mockAssessment = {
          ...
          description: "Test your knowledge of advanced React concepts including hooks, context, and performance optimization.",
          courseId: 1,
          courseName: "Advanced React and Redux: Building Scalable Web Applications",
          moduleId: 2,
          ...
          questions: [
            ...
            },
            {
              ...
              options: [
                ...
                { id: "d", text: "setState({ ...state, count: state.count + 1 })" }
              ],
              correctAnswer: "b",
              explanation: "When updating state based on the previous state, you should use the functional form of setState, which receives the previous state as an argument and returns the new state."
            },
            {
              id: 3,
              ...
              correctAnswer: "a",
              explanation: "useLocalStorage is not a built-in React hook. It's a custom hook that developers often create to handle local storage interactions. useRef, useCallback, and useMemo are all built-in React hooks."
            },
            {
              id: 4,
              ...
              options: [
                { id: "a", text: "Context provides a way to pass data through the component tree without having to pass props down manually at every level" },
                { id: "b", text: "Context should be used for low-frequency updates like theme changes or user authentication" },
                { id: "c", text: "Context is the best solution for all state management needs" },
                { id: "d", text: "Context can be consumed using the useContext hook" }
              ],
              correctAnswer: ["a", "b", "d"],
              explanation: "Context is designed for sharing data that can be considered 'global' for a tree of React components. It's ideal for low-frequency updates but not optimal for high-frequency updates where Redux or another state management library might be better. useContext is the modern way to consume context."
            },
            {
              id: 5,
              ...
              type: "single-choice",
              options: [
                { id: "a", text: "Memoizes a function to prevent unnecessary re-renders" },
                { id: "b", text: "Memoizes a value to prevent expensive recalculations" },
                { id: "c", text: "Creates a mutable ref object" },
                { id: "d", text: "Manages side effects in function components" }
              ],
              correctAnswer: "b",
              explanation: "useMemo memoizes a computed value, only recomputing it when one of the dependencies has changed. This is useful for expensive calculations to prevent them from running on every render."
            },
            {
              id: 6,
              ...
              type: "single-choice",
              options: [
                { id: "a", text: "The variables that will be available inside the effect" },
                { id: "b", text: "When the effect will run" },
                { id: "c", text: "The cleanup function that runs before the component unmounts" },
                { id: "d", text: "The order in which multiple effects are executed" }
              ],
              correctAnswer: "b",
              explanation: "The dependency array controls when the effect will run. If any value in the dependency array changes, the effect will run again. If you provide an empty array, the effect will only run once after the initial render."
            },
            {
              id: 7,
              ...
              options: [
                ...
                { id: "b", text: "Using useCallback to memoize callback functions" },
                { id: "c", text: "Using inline styles for all components" },
                { id: "d", text: "Implementing shouldComponentUpdate or React.PureComponent" }
              ],
              correctAnswer: ["a", "b", "d"],
              explanation: "React.memo, useCallback, and shouldComponentUpdate/PureComponent are all valid optimization techniques in React. Inline styles aren't generally an optimization technique and can sometimes lead to performance issues when overused."
            },
            {
              id: 8,
              ...
              type: "single-choice",
              options: [
                { id: "a", text: "const [state, setState] = useState(this.props.initialValue)" },
                { id: "b", text: "const [state, setState] = useState(props.initialValue)" },
                { id: "c", text: "const state = { value: props.initialValue }" },
                { id: "d", text: "const state = new State(props.initialValue)" }
              ],
              correctAnswer: "b",
              explanation: "In function components, you can directly access props as a parameter. The correct way to initialize state with props is to pass the prop value to the useState hook."
            },
            {
              id: 9,
              ...
              options: [
                ...
                { id: "b", text: "To lazy load components and wait for them to load" },
                { id: "c", text: "To optimize performance by suspending unnecessary renders" },
                { id: "d", text: "To create animations and transitions" }
              ],
              correctAnswer: "b",
              explanation: "React Suspense lets your components 'wait' for something before rendering. Currently, Suspense only supports one use case: loading components dynamically with React.lazy. In the future, it will support other use cases like data fetching."
            },
            {
              id: 10,
              ...
              type: "multiple-choice",
              options: [
                { id: "a", text: "Hooks can be called inside loops and conditions" },
                { id: "b", text: "Hooks can only be called at the top level of a component" },
                { id: "c", text: "Hooks can only be called from React function components" },
                { id: "d", text: "Hooks can be used inside class components" }
              ],
              correctAnswer: ["b", "c"],
              explanation: "The Rules of Hooks state that hooks should only be called at the top level (not inside loops, conditions, or nested functions) and only from React function components or custom hooks. They cannot be used in class components."
            }
          ]
        };
    ...
    if (question.type === 'single-choice') {
      ...
      setUserAnswers(prev => {
        ...
        if (currentSelections.includes(selectedOption)) {
          ...
          return {
            ...prev,
            [questionId]: currentSelections.filter(opt => opt !== selectedOption)
          };
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const scorePercentage = Math.round((correctAnswers / assessment.questions.length) * 100);
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-4 py-5 sm:p-6">
              <h1 className="text-2xl font-bold text-gray-900 text-center">Assessment Results</h1>
              <h2 className="text-xl text-gray-700 mt-2 text-center">{assessment.title}</h2>
              ...
              <div className="mt-8 max-w-lg mx-auto">
                <div className="bg-gray-50 border rounded-lg p-6 text-center">
                  ...
                  <div className={`inline-flex items-center px-4 py-2 rounded-full text-base font-medium ${results.passed ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-6">
                    {assessment.questions.map((question, index) => {
                      const questionResult = results.detailedResults.find(r => r.questionId === question.id);
                      ...
                      return (
                        <div key={question.id} className={`border rounded-lg p-4 ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                          <div className="flex items-start">
                            <span className={`flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-full text-xs font-medium ${isCorrect ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'} mr-3 mt-0.5`}>
                              {index + 1}
                            </span>
                            <div className="flex-1">
                              <p className="text-base font-medium text-gray-900">{question.text}</p>
                                ...
                                {question.options.map(option => {
                                  const isSelectedByUser = question.type === 'single-choice' 
                                    ? questionResult.userAnswer === option.id
                                    : (questionResult.userAnswer || []).includes(option.id);
                                  
                                  const isCorrectOption = question.type === 'single-choice'
                                    ? question.correctAnswer === option.id
                                    : (question.correctAnswer || []).includes(option.id);
                                  
                                  let optionClasses = "pl-9 py-2 px-3 rounded-md flex items-start";
                                  ...
                                  if (isSelectedByUser && isCorrectOption) {
                                    ...
                                    optionClasses += " bg-green-100 border border-green-300";
                                  } else {
                                  ...
                                  return (
                                    <div key={option.id} className={optionClasses}>
                                      <div className="flex items-center h-5 absolute ml-[-28px]">
                                        {isSelectedByUser && (
                                          <svg className={`h-5 w-5 ${isCorrectOption ? 'text-green-500' : 'text-red-500'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            {isCorrectOption ? (
        ...
        <div className="bg-white shadow rounded-lg mb-6">
          <div className="px-4 py-5 sm:px-6 flex justify-between items-center flex-wrap">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">{assessment.courseName} • {assessment.moduleName}</p>
            </div>
            
            <div className="flex items-center mt-2 sm:mt-0">
              {timeLeft !== null && (
                <div className={`flex items-center mr-6 ${timeLeft < 300 ? 'text-red-600' : 'text-gray-700'}`}>
              ...
              <div className="flex items-center">
                <svg className="h-5 w-5 mr-1 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-start">
              ...
              <span className="text-xs text-gray-500">
                {currentQuestion.type === 'single-choice' ? 'Select one answer' : 'Select all that apply'}
              </span>
            </div>
              ...
              {currentQuestion.options.map(option => {
                ...
                return (
                  <div key={option.id} className="relative flex">
                    ...
                    <label htmlFor={`option-${option.id}`} className="ml-3 block text-sm font-medium text-gray-700 cursor-pointer">
                      {option.text}
                    </label>
                  </div>
                );
