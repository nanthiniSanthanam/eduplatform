...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch(`/api/admin/users/?page=${userPage}&role=${userRoleFilter}&search=${userSearchQuery}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch(`/api/admin/courses/?page=${coursePage}&status=${courseStatusFilter}&search=${courseSearchQuery}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-2 text-sm text-gray-600">
            <span className="text-green-600 font-medium">↑ 12%</span> from previous month
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-2 text-sm text-gray-600">
            <span className="text-green-600 font-medium">↑ 8%</span> from previous month
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="mt-2 text-sm text-gray-600">
            <span className="text-green-600 font-medium">↑ 15%</span> from previous month
          </div>
        </div>
        
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-medium text-gray-900">Revenue</h3>
          <p className="text-3xl font-bold text-primary-600 mt-2">${stats.revenue.toFixed(2)}</p>
          <div className="mt-2 text-sm text-gray-600">
            <span className="text-green-600 font-medium">↑ 10%</span> from previous month
          </div>
        </div>
      </div>
              ...
              <div key={index} className="py-3 border-b border-gray-200 last:border-0">
                <div className="flex items-start">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${
                    ...
                    activity.type === 'course_created' ? 'bg-blue-100 text-blue-600' :
  ...
  const renderUsersTab = () => (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h3 className="text-xl font-medium text-gray-900 mb-3 md:mb-0">User Management</h3>
              ...
              <tr key={user.id}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="h-10 w-10 flex-shrink-0">
                      <img className="h-10 w-10 rounded-full" src={user.avatar || "https://via.placeholder.com/100"} alt="" />
                    </div>
                    <div className="ml-4">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex justify-end space-x-2">
                    <Link to={`/admin/users/${user.id}`} className="text-primary-600 hover:text-primary-900">
                      View
                    </Link>
      ...
      <nav className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 mt-4">
        <div className="hidden sm:block">
          <p className="text-sm text-gray-700">
            Showing <span className="font-medium">{(userPage - 1) * usersPerPage + 1}</span> to <span className="font-medium">{Math.min(userPage * usersPerPage, users.length + (userPage - 1) * usersPerPage)}</span> of <span className="font-medium">many</span> results
          </p>
        </div>
        <div className="flex-1 flex justify-between sm:justify-end">
          <button
            onClick={() => { if(userPage > 1) { setUserPage(userPage - 1); } }}
            disabled={userPage <= 1}
  ...
  const renderCoursesTab = () => (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h3 className="text-xl font-medium text-gray-900 mb-3 md:mb-0">Course Management</h3>
        <div className="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-3">
          <select 
          ...
          <div key={course.id} className="bg-white border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="p-4">
              ...
              <div className="flex items-center mb-3">
                <img 
                  src={course.instructor.avatar || "https://via.placeholder.com/40?text=I"} 
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {course.status === 'published' && (
                  <button 
                    onClick={() => handleCourseStatusChange(course.id, 'featured')}
                ...
                {course.status !== 'archived' && (
                  <button 
                    onClick={() => handleCourseStatusChange(course.id, 'archived')}
      ...
      <nav className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 mt-6">
        <div className="hidden sm:block">
          <p className="text-sm text-gray-700">
            Showing <span className="font-medium">{(coursePage - 1) * coursesPerPage + 1}</span> to <span className="font-medium">{Math.min(coursePage * coursesPerPage, courses.length + (coursePage - 1) * coursesPerPage)}</span> of <span className="font-medium">many</span> results
          </p>
        </div>
        <div className="flex-1 flex justify-between sm:justify-end">
          <button
            onClick={() => { if(coursePage > 1) { setCoursePage(coursePage - 1); } }}
  ...
  const renderReportsTab = () => (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-xl font-medium text-gray-900 mb-6">Platform Reports</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="border rounded-lg p-4">
          <h4 className="text-lg font-medium text-gray-800 mb-4">Enrollments Over Time</h4>
      ...
      <div className="border rounded-lg p-4">
        <div className="flex justify-between items-center mb-4">
          <h4 className="text-lg font-medium text-gray-800">Top Performing Courses</h4>
              ...
              {[1, 2, 3, 4, 5].map((_, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="h-10 w-10 flex-shrink-0">
                        <img className="h-10 w-10 rounded object-cover" src={`https://via.placeholder.com/100?text=${index + 1}`} alt="" />
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">
                          Top Course {index + 1}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">Instructor {index + 1}</div>
                  </td>
  ...
  const renderSettingsTab = () => (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-xl font-medium text-gray-900 mb-6">Platform Settings</h3>
      ...
      <div className="space-y-6">
        ...
        <div className="border-b pb-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h5 className="text-sm font-medium text-gray-700">Allow Public Registration</h5>
                <p className="text-xs text-gray-500">Allow new users to register without approval</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h5 className="text-sm font-medium text-gray-700">Require Email Verification</h5>
                <p className="text-xs text-gray-500">Require users to verify their email before accessing content</p>
              </div>
              <div className="flex items-center h-6">
                <input
            ...
            <div className="flex items-center justify-between">
              <div>
                <h5 className="text-sm font-medium text-gray-700">Allow Instructor Registration</h5>
                <p className="text-xs text-gray-500">Allow users to register as instructors</p>
              </div>
              <div className="flex items-center h-6">
                <input
        ...
        <div className="border-b pb-6">
          <h4 className="text-lg font-medium text-gray-800 mb-4">Payment Settings</h4>
        ...
        <div className="border-b pb-6">
          <h4 className="text-lg font-medium text-gray-800 mb-4">Appearance Settings</h4>
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center">
          <div className="mr-4">
            ...
            <span className="ml-1 font-medium text-gray-900">{currentUser.first_name} {currentUser.last_name}</span>
          </div>
          <img 
