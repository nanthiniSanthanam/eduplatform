        ...
        const mockDiscussions = [
          {
            ...
            isPinned: true,
            isAnswered: true,
            content: "I'm working on a large application and noticing some performance issues when updating my Redux state. I've read about memoization and the useSelector hook, but I'm still seeing render issues. Any advice on best practices or patterns for optimizing Redux state updates in large applications?",
          ...
          },
          {
            ...
            isPinned: false,
            isAnswered: true,
            content: "In Module 3, we briefly covered React Suspense for code splitting, but I'm curious about using it for data fetching as well. The documentation mentions experimental features for data fetching with Suspense. Has anyone experimented with this in a production environment? What libraries are you using with it?",
          ...
          },
          {
            ...
            isPinned: false,
            isAnswered: false,
            content: "I'm having trouble with the second assignment where we need to implement a custom middleware for API calls. I've set up the middleware correctly (I think), but I'm getting an error: 'Error: Actions must be plain objects. Use custom middleware for async actions.' Can someone point me in the right direction? Here's my code:\n\n```javascript\nconst apiMiddleware = store => next => action => {\n  if (action.type !== 'API_REQUEST') {\n    return next(action);\n  }\n  \n  // Handle API request\n  const { url, method, data, onSuccess, onFailure } = action.payload;\n  \n  // Rest of the middleware...\n};\n```",
          ...
          },
          {
            ...
            isPinned: false,
            isAnswered: true,
            content: "In Module 6, we covered testing React components with Jest and React Testing Library. I'm curious about best approaches for testing Redux specifically. Are there any recommended patterns or libraries for testing Redux actions, reducers, and connected components? How do you handle testing async actions?",
          ...
          },
          {
            ...
            author: {
              id: 106,
              name: "Robert Johnson",
              avatar: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=facearea&facepad=2&w=100&h=100&q=80"
            },
            category: "technical",
            ...
            isPinned: false,
            isAnswered: true,
            content: "I'm trying to deploy my project to Netlify but keep getting build errors. The app works fine locally, but the build fails with: 'Failed to compile. Module not found: Error: Can't resolve './styles/main.css'. Has anyone else encountered this issue? How did you fix it?",
          ...
          },
          {
            ...
            author: {
              id: 107,
              name: "Emily Chen",
              avatar: "https://images.unsplash.com/photo-1489424731084-a5d8b219a5bb?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=facearea&facepad=2&w=100&h=100&q=80"
            },
            category: "projects",
            ...
            isPinned: false,
            isAnswered: false,
            content: "For those working in teams for the final project, what collaboration tools are you using? We're using GitHub for version control, but I'm curious about project management tools, communication platforms, and any other tools that might help with team coordination.",
          ...
          },
          {
            ...
            isPinned: false,
            isAnswered: true,
            content: "I noticed that our course materials are using React 17, but React 18 has been out for a while now. I'm curious about the new features in React 18 like automatic batching, concurrent rendering, and the new suspense SSR architecture. Will we be covering any of these topics in the later modules?",
          ...
          replies: [
            {
              ...
              content: "I've been using the Redux Toolkit which has really simplified a lot of this. It includes utilities like createSlice that automatically generate action creators and handle immutable updates. Have you tried it?\n\nAlso, make sure you're not doing deep comparisons or complex operations in your selectors. The useSelector hook with reselect can help a lot with memoization.",
            ...
            },
            {
              ...
              content: "Thank you all for the helpful responses! I've started implementing Redux Toolkit and normalizing my state structure, and I'm already seeing performance improvements. Looking forward to Module 5 to learn more optimization techniques.",
          ...
          <div className="mb-6">
            <nav className="flex" aria-label="Breadcrumb">
              <ol className="flex items-center space-x-2">
                ...
                <li>
                  <Link to={`/courses/${courseId}/discussions`} className="text-gray-500 hover:text-gray-700">
                    Discussions
                  </Link>
                </li>
                <li>
                ...
                <li className="text-gray-700 font-medium truncate" style={{ maxWidth: '300px' }}>
              ...
              <div className="flex items-center justify-between flex-wrap sm:flex-nowrap">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Started by {currentDiscussion.author.name} • {formatDate(currentDiscussion.createdAt)}
                  </p>
                </div>
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-6">
              <div className="flex space-x-4">
                <div className="flex-shrink-0">
                  <img className="h-10 w-10 rounded-full" src={currentDiscussion.author.avatar} alt={currentDiscussion.author.name} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-gray-900">
                    {currentDiscussion.author.name}
                  </div>
                  <div className="mt-1 text-sm text-gray-700 prose max-w-none" dangerouslySetInnerHTML={{ __html: currentDiscussion.content.replace(/\n/g, '<br>') }} />
                  
                  {currentDiscussion.tags && currentDiscussion.tags.length > 0 && (
                    <div className="mt-4 flex flex-wrap">
                      {currentDiscussion.tags.map(tag => (
                        <span key={tag} className="mr-2 mb-2 inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-800">
                          {tag}
                        </span>
                      ))}
                    </div>
          ...
          <div className="bg-white shadow sm:rounded-lg mb-6">
            ...
            <div className="divide-y divide-gray-200">
              {currentDiscussion.replies.map(reply => (
                <div key={reply.id} className={`px-4 py-6 sm:px-6 ${reply.isAcceptedAnswer ? 'bg-green-50' : ''}`}>
                  <div className="flex space-x-4">
                    <div className="flex-shrink-0">
                      <img className="h-10 w-10 rounded-full" src={reply.author.avatar} alt={reply.author.name} />
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 text-sm text-gray-700 prose max-w-none" dangerouslySetInnerHTML={{ __html: reply.content.replace(/\n/g, '<br>') }} />
                      
                      <div className="mt-2 flex items-center space-x-4">
                        <button className={`flex items-center text-sm ${reply.likes > 0 ? 'text-primary-600' : 'text-gray-400 hover:text-gray-500'}`}>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
                          </svg>
            ...
            <div className="px-4 py-6 sm:px-6 border-t border-gray-200">
              <div className="flex space-x-4">
                <div className="flex-shrink-0">
                  <img className="h-10 w-10 rounded-full" src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=facearea&facepad=2&w=100&h=100&q=80" alt="Your avatar" />
                </div>
                <div className="min-w-0 flex-1">
                  <form onSubmit={handleReply}>
                    <div>
                      <label htmlFor="reply" className="sr-only">Reply to discussion</label>
                      <textarea
                        id="reply"
                        name="reply"
                        rows={3}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd>
                      <div className="text-lg font-medium text-gray-900">
                        {discussions.length > 0 ? formatDate(discussions[0].updatedAt) : 'No activity'}
                      </div>
                    </dd>
                  </dl>
                </div>
        ...
        {showNewDiscussionForm && (
          <div className="bg-white shadow rounded-lg my-6">
            <div className="px-4 py-5 sm:p-6">
              <h2 className="text-lg font-medium text-gray-900 mb-4">Create New Discussion</h2>
                  ...
                  <div className="flex justify-end">
                    ...
                    <button
                      ...
                      className={`inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 ${
                        isCreatingDiscussion ? 'opacity-70 cursor-not-allowed' : ''
                      }`}
                    >
          ...
          <div className="divide-y divide-gray-200">
            {filteredDiscussions.length === 0 ? (
              ...
              filteredDiscussions.map(discussion => (
                <div key={discussion.id} className={`px-4 py-5 sm:px-6 hover:bg-gray-50 ${discussion.isPinned ? 'bg-yellow-50' : ''}`}>
                  <div className="flex items-center justify-between">
                    <Link to={`/courses/${courseId}/discussions/${discussion.id}`} className="flex-1 min-w-0">
                      <div className="flex items-start">
                        <div className="flex-shrink-0">
                          <img className="h-10 w-10 rounded-full" src={discussion.author.avatar} alt={discussion.author.name} />
                        </div>
...
Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredDiscussions.length}</span> of <span className="font-medium">{filteredDiscussions.length}</span> discussions
</p>
</div>
<div>
