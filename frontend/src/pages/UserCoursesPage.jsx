...
const UserCoursesPage = () => {
      ...
      try {
        ...
        const mockCourses = [
          ...
          },
          {
            ...
            thumbnail: 'https://images.unsplash.com/photo-1639322537228-f710d846310a?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&q=80',
            category: 'Blockchain',
            progress: 0,
  ...
  const filteredCourses = courses.filter(course => {
    ...
    if (sortBy === 'recent') {
      return new Date(b.lastAccessed || b.enrollmentDate) - new Date(a.lastAccessed || a.enrollmentDate);
  ...
  const courseStats = {
    ...
    notStarted: courses.filter(course => course.status === 'not-started').length
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="mt-4 flex md:mt-0">
              <Link to="/courses" className="ml-3 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-primary-700 bg-white hover:bg-primary-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white">
                Browse More Courses
              </Link>
            </div>
          </div>
              ...
              <nav className="-mb-px flex">
                {['all', 'in-progress', 'completed', 'not-started'].map((tab) => (
                  ...
                  >
                    ...
                    <span className="ml-1 text-gray-400">({tab === 'all' ? courseStats.total : 
                                                          tab === 'in-progress' ? courseStats.inProgress : 
                                                          tab === 'completed' ? courseStats.completed : 
                                                          courseStats.notStarted})</span>
                  </button>
                ))}
              </nav>
            </div>
            ...
            {filteredCourses.length === 0 ? (
              ...
              <ul className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {filteredCourses.map((course) => (
                  <li key={course.id} className="col-span-1 bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
                    ...
                    <div className="p-5">
                        ...
                        <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full ${
                              ...
                              course.progress > 60 ? 'bg-primary-500' : 
                              course.progress > 30 ? 'bg-yellow-500' : 
                              course.progress > 0 ? 'bg-orange-500' : 
                              'bg-gray-300'
                            }`} 
                      ...
                      <div className="mt-5 border-t border-gray-200 pt-4">
                        <div className="flex justify-between text-xs text-gray-500">
                          <span>
                            {course.status === 'completed' ? 'Completed on:' : 
                             course.status === 'not-started' ? 'Enrolled on:' : 
                             'Last accessed:'}
                          </span>
                          <span>{formatDate(course.completionDate || course.lastAccessed || course.enrollmentDate)}</span>
                        </div>

                        {course.status === 'in-progress' && (
                          <div className="mt-2 text-xs text-gray-500 flex justify-between">
                            <span>Next:</span>
                            <span title={course.nextLesson} className="truncate" style={{ maxWidth: '70%' }}>
                              {course.nextLesson}
                            </span>
                          </div>
                        )}
                        ...
                        {course.dueDate && course.status !== 'completed' && (
                          <div className="mt-2 text-xs flex justify-between">
                            ...
                            <span className={`${calculateDaysLeft(course.dueDate) < 7 ? 'text-red-600 font-medium' : 'text-gray-500'}`}>
                              {formatDate(course.dueDate)}
                              {calculateDaysLeft(course.dueDate) < 7 && (
                                <span className="ml-1">
                                  ({calculateDaysLeft(course.dueDate)} days left)
                                </span>
                              )}
                            </span>
                          </div>
