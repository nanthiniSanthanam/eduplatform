...
const InstructorCourseDetailPage = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const courseData = Array.isArray(courseResponse) ? courseResponse[0] : courseResponse;
        if (!courseData) {
  ...
  const handlePublish = async () => {
    try {
      ...
      alert('Failed to publish course. Please ensure all required fields are filled.');
    }
  };
  ...
  if (error) {
    return (
      <Container>
        <Alert type="error" className="my-8">{error}</Alert>
        <Button 
      ...
      <Container>
        <div className="mb-6">
          <div className="flex items-center mb-1">
            <Link to="/instructor/dashboard" className="text-blue-600 hover:underline text-sm">
              Dashboard
            </Link>
          ...
          <p className="text-gray-600 mt-2">{course.description || course.subtitle}</p>
        </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <Button 
                variant="contained" 
                color="primary"
                onClick={() => navigate(`/courses/${courseSlug}/modules/create`)}
              >
                Add New Module
              </Button>
            </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {module.lessons?.length || 0} lessons · {module.duration || 'No duration set'}
                          </p>
                        </div>
                        <div className="flex space-x-2">
                          <Button
                            ...
                            color="secondary"
                            size="small"
                            onClick={() => navigate(`/courses/modules/${module.id}/edit`)}
                          >
                            Edit Module
                          </Button>
                          <Button
                            ...
                            color="primary"
                            size="small"
                            onClick={() => navigate(`/courses/${courseSlug}/modules/${module.id}/lessons/create`)}
                          >
                            Add Lesson
                          </Button>
                        </div>
                        ...
                        {module.lessons && module.lessons.length > 0 ? (
                          <ul className="divide-y divide-gray-200 border border-gray-200 rounded-md overflow-hidden">
                            {module.lessons.map((lesson) => (
                              <li key={lesson.id} className="p-3 hover:bg-gray-50">
                                <div className="flex justify-between items-center">
                                  ...
                                  <div className="space-x-2">
                                    <Button
                                      ...
                                      color="primary"
                                      size="small"
                                      onClick={() => navigate(`/courses/${courseSlug}/content/${module.id}/${lesson.id}`)}
                                    >
                                      View
                                    </Button>
                                    <Button
        ...
        {activeTab === 'lessons' && (
          <div>
            ...
            {modules.length === 0 ? (
              <div className="text-gray-600">No modules found. Please add a module first.</div>
            ) : (
              modules.map((module) => (
                <Card key={module.id} className="mb-4">
                  <div className="p-4">
                    ...
                    {module.lessons && module.lessons.length > 0 ? (
                      <ul className="divide-y divide-gray-200 border border-gray-200 rounded-md overflow-hidden">
                        {module.lessons.map((lesson) => (
                          <li key={lesson.id} className="p-3 hover:bg-gray-50 flex justify-between items-center">
                            <div>
                              <span className="font-medium">{lesson.title}</span>
                              <span className="ml-2 text-sm text-gray-600">
                                ({lesson.type}, {lesson.duration || 'No duration'})
                      ...
                      <div className="text-gray-500 italic">No lessons in this module yet.</div>
                    )}
                  </div>
                </Card>
              ))
                      ...
                      module.lessons.map((lesson) => (
                        <div key={lesson.id} className="mb-2">
                          ...
                          {lesson.resources && lesson.resources.length > 0 ? (
                            ...
                            <div className="ml-4 text-gray-500 italic">No resources for this lesson.</div>
                          )}
                        </div>
                      ))
                    ) : (
                          ...
                          {lesson.assessments && lesson.assessments.length > 0 ? (
                            <ul className="ml-4 list-disc">
                              {lesson.assessments.map((assess) => (
                                <li key={assess.id} className="flex justify-between items-center">
                            ...
                            <div className="ml-4 text-gray-500 italic">No assessments for this lesson.</div>
                          )}
                        </div>
                      ))
                    ) : (
          ...
          <div>
            <Card className="mb-4">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Current status: <span className={course.is_published ? 'text-green-600' : 'text-yellow-600'}>
