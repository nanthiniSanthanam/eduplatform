 ...
 * - Visual indicators for content access levels (basic/intermediate/advanced)
...
const CourseLandingPage = () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        if (isInstructorView) {
          ...
          try {
            const instructorResponse = await instructorService.getCourseBySlug(courseSlug);
            // Check if we have data in the response or in response.data
            courseData = instructorResponse.data || instructorResponse;
            console.log('Course fetched successfully via instructor service:', courseData);
          } catch (instructorError) {
            console.error('Error fetching with instructorService:', instructorError);
            // Fall back to regular course service
            const regularResponse = await courseService.getCourseBySlug(courseSlug);
  ...
  const handlePublishToggle = async () => {
    try {
      ...
      alert(`Course ${newPublishState ? 'published' : 'unpublished'} successfully`);
    } catch (err) {
      console.error('Failed to toggle publish status:', err);
      alert(`Failed to ${course.is_published ? 'unpublish' : 'publish'} course. Please ensure all required fields are filled.`);
    }
  };
  ...
  const getAccessLevelIcon = (accessLevel) => {
    switch(accessLevel || 'basic') {
      case 'advanced':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" 
               className="w-3.5 h-3.5 text-amber-500" aria-label="Premium content">
      ...
      case 'intermediate':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" 
               className="w-3.5 h-3.5 text-blue-500" aria-label="Registered users">
      ...
      default: // basic
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" 
               className="w-3.5 h-3.5 text-green-500" aria-label="Free content">
      ...
      course.modules.forEach(module => {
        if (module.lessons && module.lessons.length) {
          module.lessons.forEach(lesson => {
            ...
            console.log(`  - metadata?.accessLevel: ${lesson.metadata?.accessLevel}`);
            console.log(`  - metadata?.access_level: ${lesson.metadata?.access_level}`);
          ...
          <div className="container mx-auto py-6 px-4">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
              <div className="flex items-center mb-4">
                <span className={`inline-block w-3 h-3 rounded-full mr-2 ${course.is_published ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
                <span className="font-medium">{course.is_published ? 'Published' : 'Draft'}</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-1">Students Enrolled</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    {course.modules && course.modules.length > 0 ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <Button 
                          color="primary"
                          className="mt-4"
                          onClick={() => navigate(`/courses/${course.slug}/modules/create`)}
                        >
                          Create First Module
                        </Button>
                      </div>
              ...
              <div>
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        ...
                        style={{ width: `${calculateCompletionPercentage(course)}%` }}
                      ></div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className={`flex items-center ${course.modules?.length ? 'text-green-600' : 'text-gray-400'}`}>
                      <i className={`fas fa-${course.modules?.length ? 'check-circle' : 'circle'} mr-2`}></i>
                      <span>Add modules ({course.modules?.length || 0})</span>
                    </div>
                    <div className={`flex items-center ${hasLessons(course) ? 'text-green-600' : 'text-gray-400'}`}>
                      <i className={`fas fa-${hasLessons(course) ? 'check-circle' : 'circle'} mr-2`}></i>
                      <span>Add lessons</span>
                    </div>
                    <div className={`flex items-center ${course.thumbnail ? 'text-green-600' : 'text-gray-400'}`}>
                      <i className={`fas fa-${course.thumbnail ? 'check-circle' : 'circle'} mr-2`}></i>
                      <span>Upload thumbnail</span>
                    </div>
                    <div className={`flex items-center ${course.price ? 'text-green-600' : 'text-gray-400'}`}>
                      <i className={`fas fa-${course.price ? 'check-circle' : 'circle'} mr-2`}></i>
              ...
              <aside className="md:col-span-1">
                <AnimatedElement type="fade-in" delay={100}>
                  <section className="mb-8">
                    <h3 className="text-xl font-bold mb-4 font-display">Course Curriculum</h3>
                    {/* Reduced width sidebar */}
                    <div className="bg-white p-4 rounded-xl shadow-sm max-w-xs">
                      {course?.modules?.length > 0 ? (
                        <ul className="space-y-2">
                          {course.modules.map((module, mIdx) => {
                            const moduleAccessLevel = module.accessLevel || module.access_level || 'basic';
                                ...
                                <details open={mIdx === 0} className="mb-2">
                                  <summary className="cursor-pointer font-medium text-primary-700 flex items-center justify-between p-2 rounded hover:bg-gray-50">
                                    <div className="flex items-center">
                                      ...
                                      <span className="ml-2 flex-1">{module.title}</span>
                                    </div>
                                  </summary>
                                  ...
                                  <ul className="mt-2 space-y-1 border-l-2 border-gray-100 pl-3 ml-2">
                                    {module.lessons && module.lessons.map((lesson, lIdx) => {
                                      ...
                                      const lessonAccessLevel = 
                                        lesson.accessLevel || 
                                        lesson.access_level || 
                                        (lesson.metadata && (lesson.metadata.accessLevel || lesson.metadata.access_level)) || 
                                        moduleAccessLevel || 
                                        'basic';
                                      
                                      // Log access level for debugging
                                      console.debug(`Rendering lesson ${lesson.id}: ${lesson.title} - Access Level: ${lessonAccessLevel}`);
                        ...
                        <div className="text-gray-500 text-center py-8">No modules available yet.</div>
                      )}
                    </div>
                    
                    {/* Access level legend */}
                    <div className="mt-4 bg-gray-50 p-3 rounded-lg max-w-xs">
                      <p className="text-sm font-medium text-gray-700 mb-2">Content Access Levels:</p>
                      <div className="space-y-2 text-sm">
                        <div className="flex items-center">
                          {getAccessLevelIcon('basic')}
                          <span className="ml-2 text-gray-600">Free Content</span>
                        </div>
                        <div className="flex items-center">
                          {getAccessLevelIcon('intermediate')}
                          <span className="ml-2 text-gray-600">Requires Registration</span>
                        </div>
                        <div className="flex items-center">
                          {getAccessLevelIcon('advanced')}
                          <span className="ml-2 text-gray-600">Premium Content</span>
                        </div>
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="mb-8">
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-sm max-w-xs">
                      {course?.instructors?.map(instructor => (
                        <div key={instructor?.instructor?.id || Math.random()} className="mb-4 last:mb-0">
                          <div className="flex items-center mb-3">
                            ...
                            <div>
                              <h4 className="font-medium">
                                {instructor?.instructor?.first_name || 'Unknown'} {instructor?.instructor?.last_name || 'Instructor'}
                              </h4>
