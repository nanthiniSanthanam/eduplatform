...
const AssessmentPage = () => {
  ...
  useEffect(() => {
    const checkAccessAndAuth = async () => {
      try {
        ...
        const lessonResponse = await courseService.getLessonDetails(lessonId, true); // Use non-authenticated request
        ...
        if (accessLevel !== 'basic') {
          ...
          console.log('Free assessment content - allowing access without login');
        }
      } catch (error) {
    ...
    const fetchAssessment = async () => {
      try {
        ...
        try {
          ...
          console.log('Lesson access level:', lessonResponse.access_level || 'basic (default)');
        } catch (error) {
          // If that fails, try authenticated request (will only work if user is logged in)
          console.log('Free content fetch failed, trying authenticated request');
        ...
        if (lessonResponse.assessment) {
          console.log('Assessment found in lesson response:', lessonResponse.assessment);
          ...
          const assessmentResponse = await courseService.getAssessmentDetails(lessonResponse.assessment_id);
  ...
  const startAssessment = async () => {
    try {
      ...
      assessment.questions.forEach(question => {
        initialAnswers[question.id] = question.question_type === 'multiple_choice' ? [] : null;
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      const formattedAnswers = Object.entries(answers).map(([questionId, answer]) => ({
        ...
        answer_id: Array.isArray(answer) ? answer.map(id => parseInt(id)) : parseInt(answer)
      }));
  ...
  const handleReturnToLesson = () => {
    navigate(`/courses/${courseSlug}/content/${assessment?.lesson?.module_id || '1'}/${lessonId}`);
  };
  ...
  const calculateCompletion = () => {
    ...
    const answeredQuestions = Object.values(answers).filter(
      answer => answer !== null && (Array.isArray(answer) ? answer.length > 0 : true)
    ).length;
          ...
          <Card className="max-w-xl mx-auto p-8">
            <div className="text-center mb-8">
              ...
              <div className="text-gray-600 mb-4">
                Your score: <span className="font-medium">{results.score}%</span> 
              ...
              {results.passed ? (
                <div className="bg-green-50 text-green-700 p-4 rounded-md mb-6">
                  <p>Congratulations! You've successfully completed this assessment.</p>
                </div>
              ) : (
                <div className="bg-amber-50 text-amber-700 p-4 rounded-md mb-6">
                  <p>Don't worry! You can review the lesson material and try again.</p>
                </div>
              )}
            </div>
            
            <div className="space-y-6">
              <h2 className="text-lg font-semibold border-b pb-2">Results Review</h2>
                        ...
                        <div className="ml-2 space-y-1 text-sm">
                          {question.answers.map(answer => (
                            <div key={answer.id} className={`flex items-center ${
                              answer.is_correct ? 'text-green-800 font-medium' : userAnswer?.answer_id === answer.id ? 'text-red-800' : ''
                            }`}>
                              ...
                              {answer.answer_text}
                              {answer.is_correct && (
                                <Badge size="small" variant="success" className="ml-2">Correct</Badge>
                              )}
                            </div>
                          ))}
                        </div>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <Card className="p-6 mb-6">
              ...
              <div className="mb-6">
                <Badge variant={isMultipleChoice ? "warning" : "info"} className="text-xs">
                  {isMultipleChoice ? 'Select multiple answers' : 'Select one answer'}
                </Badge>
              </div>
              ...
              <div className="space-y-3">
                {currentQuestion.answers.map((answer) => {
                  ...
                  return (
                    <button
                      key={answer.id}
                      onClick={() => handleAnswerChange(currentQuestion.id, answer.id, isMultipleChoice)}
                    ...
                    >
                      <div className="flex items-center">
                        <div className={`flex-shrink-0 h-5 w-5 mr-2 rounded ${
            ...
            <div className="mt-8 bg-white rounded-lg shadow-sm p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {assessment.questions.map((question, index) => {
                  const questionIsAnswered = answers[question.id] !== null && 
